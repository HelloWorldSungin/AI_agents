...
class AgentComposer:
    """Composes agent prompts from modular components with advanced tool use support."""

    def __init__(self, library_path: Path, project_path: Path):
        self.library_path = library_path
        self.project_path = project_path
        self.deferred_skills_manifest = []  # Track deferred skills for skill search
    ...
    def parse_skills_config(self, skills_config) -> Dict:
        """
        Parse skills configuration supporting both legacy and new formats.

        Legacy format: ["skill1", "skill2"]
        New format: {"always_loaded": [...], "deferred": [...]}

        Args:
            skills_config: Skills configuration from agent config

        Returns:
            Dictionary with 'always_loaded' and 'deferred' skill lists
        """
        ...
        if isinstance(skills_config, list):
            return {'always_loaded': skills_config, 'deferred': []}

        # New format: object with always_loaded and deferred
        if isinstance(skills_config, dict):
            return {
                'always_loaded': skills_config.get('always_loaded', []),
                'deferred': skills_config.get('deferred', [])
            }

        return {'always_loaded': [], 'deferred': []}

    def parse_deferred_skill(self, skill) -> Dict:
        """
        Parse a deferred skill entry (string or object).

        Args:
            skill: Either a string path or object with path/description/triggers

        Returns:
            Dictionary with path, description, and triggers
        """
        if isinstance(skill, str):
            return {
                'path': skill,
                'description': f"Skill: {skill.split('/')[-1]}",
                'triggers': []
            }
        ...
        if isinstance(skill, dict):
            return {
                'path': skill.get('path', ''),
                'description': skill.get('description', f"Skill: {skill.get('path', '').split('/')[-1]}"),
                'triggers': skill.get('triggers', [])
            }

        return {'path': '', 'description': '', 'triggers': []}

    def generate_skill_search_manifest(self, deferred_skills: List) -> str:
        """
        Generate a manifest of deferred skills for skill search functionality.

        Args:
            deferred_skills: List of deferred skill configurations

        Returns:
            Markdown content describing available deferred skills
        """
        ...
        manifest_lines = [
            ...
            "",
            "The following skills can be loaded on-demand when needed.",
            "Request a skill by saying: \"I need to use the [skill name] skill\"",
            "",
        ...
        for skill in deferred_skills:
            parsed = self.parse_deferred_skill(skill)
            skill_name = parsed['path'].split('/')[-1]
            description = parsed['description']
            triggers = ', '.join(parsed['triggers']) if parsed['triggers'] else 'N/A'
            manifest_lines.append(f"| {skill_name} | {description} | {triggers} |")

            # Track for JSON manifest
            self.deferred_skills_manifest.append({
                'name': skill_name,
                'path': parsed['path'],
        ...
        manifest_lines.extend([
            "",
            "**Note**: These skills are not loaded by default to optimize token usage.",
            "Request them when you encounter a task that requires their capabilities."
        ])
...
""")
        ...
        # 5. Load Anthropic Skills (with deferred loading support)
        if 'skills' in agent_config and agent_config['skills']:
            # Reset deferred skills manifest for this agent
            self.deferred_skills_manifest = []

            # Parse skills config (supports both legacy and new formats)
            skills_parsed = self.parse_skills_config(agent_config['skills'])
            always_loaded = skills_parsed['always_loaded']
            deferred = skills_parsed['deferred']

            # Load always_loaded skills immediately
            if always_loaded:
                components.append("# ========================================")
                components.append("# ANTHROPIC SKILLS (Always Loaded)")
                components.append("# ========================================\n")
            ...
            # Generate manifest for deferred skills (not loaded, just cataloged)
            if deferred:
                components.append("# ========================================")
                components.append("# ON-DEMAND SKILLS (Deferred Loading)")
                components.append("# ========================================\n")

                manifest = self.generate_skill_search_manifest(deferred)
                components.append(manifest)
                components.append("\n\n")

                # Calculate token savings
                deferred_token_count = 0
                for skill in deferred:
                    parsed = self.parse_deferred_skill(skill)
                    skill_path = self.resolve_skill_path(parsed['path'])
                    if skill_path:
                        content = self.load_markdown(skill_path)
                        deferred_token_count += self.count_tokens(content)

                if deferred_token_count > 0:
                    print(f"  ℹ️  Deferred {len(deferred)} skill(s), saving ~{deferred_token_count:,} tokens")
    ...
    def save_agent(self, agent_name: str, content: str, output_dir: Path, agent_config: Dict):
        ...
        # Save deferred skills manifest if any
        if self.deferred_skills_manifest:
            manifest_file = output_dir / f"{agent_name}-deferred-skills.json"
            with open(manifest_file, 'w', encoding='utf-8') as f:
                json.dump({
                    'agent': agent_name,
                    'deferred_skills': self.deferred_skills_manifest,
                    'generated_at': self._get_timestamp()
                }, f, indent=2)
            print(f"✓ Saved deferred skills manifest: {manifest_file}")
    ...
    def _suggest_reductions(self, agent_name: str, agent_config: Dict):
        ...
        if 'skills' in agent_config and agent_config['skills']:
            ...
            # If using legacy format (simple array), suggest migration
            if isinstance(agent_config['skills'], list) and len(agent_config['skills']) > 2:
                suggestions.append(f"  - 🚀 MIGRATE TO DEFERRED LOADING: You have {len(agent_config['skills'])} skills loaded upfront")
                suggestions.append(f"    Convert skills from array to object format with 'always_loaded' and 'deferred':")
                suggestions.append(f"    Example:")
                suggestions.append(f"      skills:")
                suggestions.append(f"        always_loaded: [\"{agent_config['skills'][0]}\"]")
                suggestions.append(f"        deferred: {agent_config['skills'][1:]}")
                suggestions.append(f"    Expected savings: 40-60% token reduction")
            elif always_loaded and len(always_loaded) > 2:
                suggestions.append(f"  - Consider moving some of {len(always_loaded)} always_loaded skills to deferred")
                suggestions.append(f"    Always loaded: {', '.join(always_loaded)}")
